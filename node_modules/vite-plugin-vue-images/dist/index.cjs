...
function appRelativePath(path, root) {
    return path.slice(root.length + 1);
}
// Helper for determining if `file` is within any of `dirs`
function fileInDirs(dirs, file) {
...
function stringifyImageImport({ name, path }) {
...
function resolveOptions(options, viteConfig) {
    ...
    options.dirs = (options.dirs || []).map(d => path.resolve(viteConfig.root, d));
...
const debug = {
    images: Debug__default['default']('vite-plugin-vue-images:context:images'),
    search: Debug__default['default']('vite-plugin-vue-images:context:search'),
    hmr: Debug__default['default']('vite-plugin-vue-images:context:hmr'),
};
class Context {
    constructor(options, viteConfig) {
        this.viteConfig = viteConfig;
        this._imagePaths = new Set();
        this._imageNameMap = {};
        this._imageUsageMap = {};
        ...
        this.globs = (this.dirs || []).map((d) => `${d}/**/*.${extsGlob}`);
    }
    get root() {
    ...
    resolvePathAlias(path) {
        var _a, _b, _c;
        // @ts-expect-error backward compatibility
        return resolveAlias(path, ((_b = (_a = this.viteConfig) === null || _a === void 0 ? void 0 : _a.resolve) === null || _b === void 0 ? void 0 : _b.alias) || ((_c = this.viteConfig) === null || _c === void 0 ? void 0 : _c.alias) || []);
    }
    searchImages() {
        var _a;
        debug.search(`started with: [${this.globs.join(', ')}]`);
        const root = this.root;
        const files = out$3.sync(this.globs, {
            ignore: ['node_modules'],
            onlyFiles: true,
            cwd: root,
        });
        if (!files.length && !((_a = this.options.customResolvers) === null || _a === void 0 ? void 0 : _a.length)) {
...
const debug$1 = Debug__default['default']('vite-plugin-vue-images:transform');
function makeTransform(ctx) {
        ...
        let transformed = code.replace(regex, (str, name) => {
            if (name && !name.startsWith('_')) {
                ...
                if (image) {
                    ...
                    head.push(stringifyImageImport(Object.assign(Object.assign({}, image), { name: varName })));
                    no += 1;
                    // return `\{ ${prop}: ${varName} }`;
                    return varName;
                }
...
function VitePluginImages(options = {}) {
    let ctx;
    let transformer;
    return {
...
module.exports = VitePluginImages;
